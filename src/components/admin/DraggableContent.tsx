...
const DraggableContent: React.FC<DraggableContentProps> = ({
  ...
  const { isEditMode, isReorderMode, reorderElements, updateText, selectedElementId, setSelectedElementId } = useEditMode();
  ...
  // Update draggable items when props change
  useEffect(() => {
  ...
  const handleItemClick = (e: React.MouseEvent, item: DraggableItem) => {
    e.stopPropagation();
    if (isEditMode) {
  ...
  // If not in edit mode, simply render the items
  if (!isEditMode) {
    return (
    ...
    <div className="relative space-y-2">
      <div className={className}>
        {draggableItems.map((item, index) => (
          <div 
            key={item.id}
            className={`${itemClassName} draggable-item ${isEditMode ? 'relative' : ''} ${isReorderMode ? 'cursor-move border border-dashed transition-colors rounded-md' : ''} ${selectedElementId === item.id ? 'ring-2 ring-primary/60 ring-offset-1' : ''} ${hoverIndex === index ? 'bg-primary/10 border-primary' : ''} mb-6`}
